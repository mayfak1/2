...
#define CTEST_IMPL_SECTION \
    __attribute__((used, section("__DATA, .ctest"), aligned(8)))
#else
...
#define CTEST_SETUP(sname)                                                   \
    template <>                                                              \
    void CTEST_IMPL_SETUP_FNAME(sname)(struct CTEST_IMPL_DATA_SNAME(sname) * \
                                       data)

#define CTEST_TEARDOWN(sname)                                                   \
    template <>                                                                 \
    void CTEST_IMPL_TEARDOWN_FNAME(sname)(struct CTEST_IMPL_DATA_SNAME(sname) * \
                                          data)
...
#define CTEST2_SKIP(sname, tname) CTEST_IMPL_CTEST2(sname, tname, 1)
    ...
    void assert_str(const char *cmp, const char *exp, const char *real,
...
#define ASSERT_NOT_STRSTR(str, substr) \
    ...
    void assert_wstr(const char *cmp, const wchar_t *exp, const wchar_t *real,
                     const char *caller, int line);
...
#define CTEST_DBL_EPSILON 1e-12
    ...
    void assert_compare(const char *cmp, intmax_t exp, intmax_t real,
                        const char *caller, int line);
...
#define ASSERT_GE(v1, v2) assert_compare(">=", v1, v2, __FILE__, __LINE__)
    ...
    void assert_compare_u(const char *cmp, uintmax_t exp, uintmax_t real,
                          const char *caller, int line);
...
#define ASSERT_FAIL() assert_fail(__FILE__, __LINE__)
    ...
    void assert_dbl_compare(const char *cmp, double exp, double real, double tol,
                            const char *caller, int line);
...
#define ANSI_NORMAL "\033[0m"
                    ...
                    const char *caller, int line)
    {
        ...
        {
            CTEST_ERR("%s:%d  assertion failed, '%s' %s '%s'", caller, line, exp, cmp,
                      real);
        }
    }
                     ...
                     const char *caller, int line)
    {
        ...
        {
            CTEST_ERR("%s:%d  assertion failed, '%ls' %s '%ls'", caller, line, exp,
                      cmp, real);
        }
    }
                        ...
                        const char *caller, int line)
    {
        ...
        {
            CTEST_ERR("%s:%d  assertion failed, %" PRIdMAX " %s %" PRIdMAX "", caller,
                      line, exp, cmp, real);
        }
    }
                          ...
                          const char *caller, int line)
    {
        ...
        if (!get_compare_result(cmp, c3, c3 == 0))
        {
            CTEST_ERR("%s:%d  assertion failed, %" PRIuMAX " %s %" PRIuMAX, caller,
                      line, exp, cmp, real);
        }
    }
                            ...
                            const char *caller, int line)
    {
        ...
        if (!get_compare_result(cmp, c3, eq))
        {
            ...
            CTEST_ERR("%s:%d  assertion failed, %.8g %s %.8g (diff %.4g, %s %.4g)",
                      caller, line, exp, cmp, real, diff, tolstr, tol);
        }
    }
...
#include <sys/types.h>
    ...
    __attribute__((no_sanitize_address)) int ctest_main(int argc,
                                                        const char *argv[])
    {
        static int total = 0;
        static int num_ok = 0;
